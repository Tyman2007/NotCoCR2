	...
	{	
		...
		public function levelUpGo():void {
			...
			if (player.XP >= player.requiredXP() && player.level < getGame().levelCap) {
				...
				player.level++;
				if (player.level % 3 == 0) player.perkPoints++;
				player.statPoints += 3;
				if (player.level % 6 == 0) player.ascensionPerkPoints++;
				outputText("<b>You are now level " + num2Text(player.level) + "!</b>\n\nYou have gained 3 attribute points!");
				if (player.level % 3 == 0) outputText("\n\n<b>You have also gained a perk point!</b>");
